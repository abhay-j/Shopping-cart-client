...
//   //https://shopping-cart-rest-4cb8bc3adabc.herokuapp.com
...
export function Cart({ shoppingCartResponse, user, cartItems, updateCartItemQuantity, removeCartItem }) {
  const [error, setError] = useState(null);
  const [totalPrice, setTotalPrice] = useState(0);
  const [couponCode, setCouponCode] = useState('');
  const navigate = useNavigate();
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await axios.delete(`https://shopping-cart-rest-4cb8bc3adabc.herokuapp.com/${shoppingCartResponse.cartId}/items/${itemId}`, {
        headers: {
          Authorization: `Bearer ${user.accessToken}`
        }
      });
  ...
  const handleQuantityChange = async (itemId, newQuantity) => {
    ...
    try {
      await axios.put(`https://shopping-cart-rest-4cb8bc3adabc.herokuapp.com/${shoppingCartResponse.cartId}/items/${itemId}`, 
